...
**Files:**
- Modify: `designer/js/render.js` (ajouter `buildPageStatic`, et l'import nécessaire)

- [ ] **Step 1 : Implémenter `buildPageStatic`**

Dans `designer/js/render.js`, ajouter (les builders sont dans ce fichier ; importer le registre et `placeAt`/`SCREEN`) :
```js
...
Ajouter sous les helpers (Task 1) :
```js
import { buildPageStatic } from './render.js';

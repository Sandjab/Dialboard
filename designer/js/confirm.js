...
export function withConfirm(el, action, { label = 'Confirmer ?', ms = 3000, guard } = {}) {
  ...
  let armed = false, orig = null, t = null;
  const disarm = () => { if (!armed) return; armed = false; el.classList.remove('confirm'); write(orig); if (t) { clearTimeout(t); t = null; } };
  el.addEventListener('click', e => {
    if (armed) { disarm(); action(e); return; }       // 2e clic : exécute
    if (guard && !guard()) return;                     // préconditions KO → on n'arme pas
    armed = true; orig = read(); el.classList.add('confirm'); write(label);   // 1er clic : arme
